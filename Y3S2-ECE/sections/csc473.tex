...
\mn{consider some edge in the graph with endpoints $ y_u, y_v $.
    If $ (u, v) $ is an edge ($ (u,v) \in E $), then $ y_u + y_v \ge  1 $.
    Then, $ \max \left\{  y_u, y_v \right\}  \ge  \frac{1}{2} \Rightarrow \left\{ u, v \right\}  \cap C \neq \emptyset$
}
...
\begin{equation}
    \begin{split}
        ...
        \text{subject to} \\
        \sum_{i, j \in S_}^m x_i S_i \ge 1 \text{ for all } j \in [n] \\
        x_i \in \{0, 1\} \text{ for all } i \in [m]
    \end{split}
\end{equation}
