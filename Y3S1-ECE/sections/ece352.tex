...
\begin{enumerate}
	...
	\item ori imm5
		\begin{listing}[H]
		\begin{minted}{text}
		TMP = Κ1 bitwise OR IMM5, where IMM5 is a 5 bit constant
		Κ1 = TMP
		IF (TMP == 0) ZERO = 1; ELSE ZERO = 0;
		IF (TMP < 0) NEGATIVE = 1; ELSE NEGATIVE = 0;
		PC = PC + 1
	...
	\item shift\{left, right\} r1, r2
		\begin{listing}[H]
		\begin{minted}{text}
     IF (L) ΤΗΕΝ TMP = R1 << IMM2
            ELSE TMP = R1 >> IMM2
     R1 = TMP
IF (TMP == 0) ZERO = 1; ELSE ZERO = 0;
IF (TMP < 0) NEGATIVE = 1; ELSE NEGATIVE = 0;
...
Alternative definition (equivalent to the previous one):
          ΙΜΜ3 is a 3 bit immediate in the sign – magnitude representation, i.e., bit 2 = sign, value = bits 1 and 0
          IF (IMM3 > 0) ΤΗΕΝ TMP = R1 << IMM3
                       ELSE TMP = R1 >> (-IMM3)
          R1 = TMP
IF (TMP == 0) ZERO = 1; ELSE ZERO = 0;
IF (TMP < 0) NEGATIVE = 1; ELSE NEGATIVE = 0;
PC = PC + 1
	...
	\item bz imm4
		\begin{listing}[H]
		\begin{minted}{text}
     where ΙΜΜ4 is a 2’s complement 4-bit immediate
